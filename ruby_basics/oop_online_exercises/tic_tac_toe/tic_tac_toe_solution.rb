module TicTacToe
    ...
    class Game
        def initialize(player_1_class, player_2_class)
            ...
            @players = 
                ...
                player_2_class.new(self, "O")] # What is this all about? Arguments defining type (class) of players.

            puts "#{current_player} goes first." # What is this all about? Check line 66.
        end
        attr_reader :board, :current_player_id

        def play
            loop do
                place_player_marker(current_player) # What is this all about?

                if player_has_won?(current_player) # What is this all about?
                    puts "#{current_player} wins!"
                    print_board # What is this all about?
                elsif board_full? # Check line 56 and 39.
                    puts "It's a draw."
                    print_board
                    return
                end

                switch_players! # What is this all about?
            end
        end
        ...
        def place_player_marker(player)
            position = player.select_position! # What is this all about?
            puts "#{player} selects #{player.marker} position #{position}" # Check line 96 for player.marker .
            @board[position] = player.marker
        end

        
        def player_has_won?(player)         # Are any of the LINES made up of a player marker?
            LINES.any? do |line|
        ...
        def other_player_id 
            1 - @current_player_id # Check line 20. @current_player_id can be accessed due to attr_reader.
        end
        ...
        def current_player 
            @players[current_player_id]  # Check line 13.
        end
        ...
        def turn_num
            10 - free_positions.size # What is this all about? Why 10?
        end

        def print_board
            col_separator, row_separator = " | ", "--+---+--" # Create two variables
            label_for_position = lambda{ |position| @board[position] ? @board[position] : position} # lambda?

            row_for_display = lambda {|row| row.map(&label_for_position).join(col_separator)} # What is this all about?
            row_positions = [[1,2,3], [4,5,6], [7,8,9]]
            rows_for_display = row_positions.map(&row_for_display) # What is this all about?
    ...
    class ComputerPlayer < Player
        DEBUG = false # edit this line if necessary # What is this all about?

        def group_positions_by_markers(line)
            markers = line.group_by { |position| @game.board[position]}
            markers.default = []
            markers                 # What is this all about?
        end

        def select_position!
            opponent_marker = @game.opponent.marker # What is this all about?
        ...
        def corner_trap_defense_position(opponent_marker)
            ...
            safe_responses = { 1=> [1,4], 3=>[2, 6], 7=>[4, 8], 9=>[6, 8]}
        ...
        def log_debug(message)
            puts "#{self}: #{message}" if DEBUG # What is this all about?
        end
